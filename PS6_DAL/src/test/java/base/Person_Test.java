...
public class Person_Test {
	...
	public static void setUpBeforeClass() throws Exception {
		...
		Date dDate = null;
		try {
			dDate = new SimpleDateFormat("yyyy-MM-dd").parse("1996-10-01");
		} catch (ParseException e) {
			e.printStackTrace();
		}
		
		per1 = new PersonDomainModel();
		per1.setFirstName("Hava");
		per1.setLastName("Marneweck");
		per1.setBirthday(dDate);
		per1.setCity("Newark");
		per1.setPostalCode(19717);
		per1.setStreet("Haines Street");
	}
	...
	{		
		PersonDomainModel per;
		final String C_LASTNAME = "Marneweck";
